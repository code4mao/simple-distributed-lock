...
public class ExclusiveLock implements DistributedLock {
    ...
    public ExclusiveLock() throws InterruptedException, IOException {
        ...
        lockStatusEnum = LockStatusEnum.UNLOCK;
    ...
    public Boolean tryLock() throws Exception {
        if (lockStatusEnum == LockStatusEnum.LOCKED) {
            return true;
        }

        Boolean created = createLockNode();
        lockStatusEnum = created ? LockStatusEnum.LOCKED : LockStatusEnum.UNLOCK;
        return created;
    }
    ...
    class LockNodeWatcher implements Watcher {
        ...
        public void process(WatchedEvent event) {
            ...
            } else if (Event.EventType.NodeDeleted == event.getType() && event.getPath().equals(LOCK_NODE_FULL_PATH)) {

                // 3. 再次尝试创建锁及诶单
                if (lockStatusEnum == LockStatusEnum.TRY_LOCK && createLockNode()) {
                    lockStatusEnum = LockStatusEnum.LOCKED;
                    try {
                        lockBarrier.await();
                        System.out.println("[" + id + "]" + " 获取锁");
